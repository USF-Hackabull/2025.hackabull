...
export const Navbar = () => {
  ...
  useEffect(() => {
    const banner = document.getElementById("mlh-trust-badge");

    if (!banner || !logoRef.current) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        setIsLogoHidden(!entry.isIntersecting);
      },
      {
        root: null,
        threshold: 0.1, // Trigger when at least 10% of the logo is visible
      }
    );

    // Observe the logo against the banner
        ...
        <div className={`flex items-center`}>
          <a
            ...
            className={`flex-shrink-0 transition-opacity duration-200 ${
              isLogoHidden ? "opacity-100" : "opacity-0"
            }`}
            ref={logoRef}
          >
            <div className="flex items-center">
